...
module control_unit(
	...
	input [60:0] out_signal,                                                                                    //!instruction bus from decoder
...
always@(*) begin
	case(opcode)
        ...
        7'b0000011 : begin                                                                          //! mem read set
			...
			case(out_signal) 
				...
				61'h200000 : begin
					 final_output <= mem_read[31:0];                                        //!lw
					 memory_busy <= 1;
				end
				61'h400000 :begin
						case (mem_count)
							2'b00: begin 
								final_output <= mem_read[7:0];                                         //!lbu
								memory_busy <= 1;
							end
							2'b01: begin 
            ...
            mem_count <= addr % 4;
				case(out_signal)
					61'h1000000 :begin
						case(mem_count)
							2'b00: begin 
								mem_write <= {mem_read[31:8], rs2_input[7:0]};                                      //!sb
								memory_busy <= 1;
							end
							2'b01: begin 
					...
					61'h2000000 :begin
							case(mem_count)
								2'b00: begin
									mem_write <= {mem_read[31:16], rs2_input[15:0]};                                     //!sh
									memory_busy <= 1;
								end
								2'b10: begin
        ...
        7'b1101111 : begin
			memory_busy <= 0;
		if(out_signal == 61'h200000000) begin   						  //!jal	 
				jump <= pc_input + imm;
				j_signal <= 1;  
		...
		7'b0110111 : begin
			memory_busy <= 0;
			if(j_signal==1)j_signal<=0;
			if(out_signal == 61'h800000000) begin   
        ...
        7'b0010111 : begin
			memory_busy <= 0;
			if(j_signal==1)j_signal<=0;
		if(out_signal == 61'h200000000000) begin   
	...
	7'b1110011 : begin
		case(out_signal)
			61'h40000000000000:  if (csr_addr != 12'h0)  begin 
							// Read old value and store in rdata
							csr_rdata <= csr_data[csr_addr];
							// Write new value to CSR
							csr_data[csr_addr] <= rs1_input;
						end
			61'h80000000000000:  if (csr_addr != 12'h0) begin
							// Read old value and store in rdata
							csr_rdata <= csr_data[csr_addr];
							// Set bits in CSR
							csr_data[csr_addr] <= csr_data[csr_addr] | rs1_input;
						end
			61'h100000000000000:  if (csr_addr != 12'h0) begin
							// Read old value and store in rdata
							csr_rdata <= csr_data[csr_addr];
							// Clear bits in CSR
							csr_data[csr_addr] <= csr_data[csr_addr] & ~rs1_input;
						end
			61'h200000000000000: if (csr_addr != 12'h0) begin
							// Read old value and store in rdata
							csr_rdata <= csr_data[csr_addr];
							// Write immediate value to CSR
							csr_data[csr_addr] <= imm;
						end
			61'h400000000000000:  if (csr_addr != 12'h0) begin
							// Read old value and store in rdata
							csr_rdata <= csr_data[csr_addr];
							// Set bits in CSR with immediate value
							csr_data[csr_addr] <= csr_data[csr_addr] | imm;
						end
			61'h800000000000000: if (addr != 12'h0) begin
							// Read old value and store in rdata
	...
	7'b0001111 : begin
		if (out_signal == 61'h1000000000000000) begin         // Activate fence logic when FENCE instruction is detected
			fence_active <= 1'b1;
			end
	end
		endcase
