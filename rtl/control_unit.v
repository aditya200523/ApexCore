...
module control_unit(
	...
	input [59:0] out_signal,                                                                                    //!instruction bus from decoder
...
always@(*) begin
	case(opcode)
        ...
        7'b0000011 : begin                                                                          //! mem read set
			...
			case(out_signal) 
				...
				60'h200000 : final_output <= mem_read[31:0];                                        //!lw
				60'h400000 :begin
						case (mem_count)
        ...
        7'b0100011 : begin
            ...
            mem_count <= addr % 4;
				case(out_signal)
					...
					60'h4000000 : mem_write <= rs2_input[31:0];													//!sw
				endcase
				if(j_signal==1)j_signal<=0;
        		end
        ...
        7'b1101111 : begin
		if(out_signal == 60'h200000000) begin   						  //!jal	 
				jump <= pc_input + imm;
				j_signal <= 1;  
		...
		if(out_signal == 60'h400000000) begin			
				jump <= rs1_input + imm;
				j_signal <= 1;  
            final_output <= pc_input + 4; 
            end
		...
		7'b0110111 : begin
			if(j_signal==1)j_signal<=0;
			if(out_signal == 60'h800000000) begin   
        ...
        7'b0010111 : begin
			if(j_signal==1)j_signal<=0;
		if(out_signal == 60'h200000000000) begin   
	...
	7'b0101111 : begin
		case(out_signal)
			60'h400000000000 : begin      //AMOSWAP
			...
			60'h800000000000 : begin   //AMOADD.W
				instructions <= 16'd1;
				v1 <= mem_read;
				v2 <= rs2_input;
			...
			60'h1000000000000: begin     //AMOAND.W
				instructions <= 16'd16;
				v1 <= mem_read;
				v2 <= rs2_input;
			...
			60'h2000000000000 : begin     //AMOOR.W
				instructions <= 16'd8;
				v1 <= mem_read;
				v2 <= rs2_input;
		    ...
		    60'h4000000000000 : begin     //AMOXOR.W
				instructions <= 16'd4;
				v1 <= mem_read;
				v2 <= rs2_input;
			...
			60'h8000000000000  : begin     //AMOMAX.W
			...
			60'h10000000000000 : begin     //AMOMIN.W
